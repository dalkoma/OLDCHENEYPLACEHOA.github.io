...
export default function TrainTracker({ user, addMemory, R }) {
  ...
  useEffect(() => {
    fetchTrains()
    const interval = setInterval(fetchTrains, 120000)
  ...
  const iconBtn = {
    width: R.minTouchTarget, height: R.minTouchTarget, borderRadius: R.sp(6), background: colors.surfaceHover,
    border: `${R.borderWidth}px solid ${colors.border}`, color: colors.textSecondary, fontSize: R.fs(12),
    cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
  }
  const labelStyle = { color: colors.textSecondary, fontSize: R.fs(11), fontWeight: 600, marginBottom: R.sp(4), display: 'block' }
  const modalOverlay = {
  ...
  const modalContent = {
    ...
    border: `${R.borderWidth}px solid ${colors.border}`, maxHeight: '85vh', overflowY: 'auto',
  }
  const inputStyle = {
  ...
  return (
    <div style={{ padding: R.sp(16) }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: R.sp(4) }}>
        <h2 style={{ color: colors.text, fontSize: R.fs(20), fontWeight: 700 }}>Train Tracker</h2>
        <button onClick={fetchTrains} disabled={loading} style={{
          ...
          borderRadius: R.sp(8), color: colors.textSecondary, fontSize: R.fs(12), cursor: 'pointer', fontFamily: 'inherit',
          minHeight: R.minTouchTarget,
      ...
      <div style={{ display: 'flex', gap: R.sp(8), marginBottom: R.sp(16) }}>
        {[['status', 'Live Status'], ['schedule', 'My Schedule']].map(([v, label]) => (
          <button key={v} onClick={() => setView(v)} style={{
            flex: 1, padding: `${R.sp(10)}px ${R.sp(8)}px`, background: view === v ? colors.primary : colors.surfaceLight,
      ...
      {error && (
        <div style={{ padding: R.sp(14), background: `${colors.danger}15`, border: `${R.borderWidth}px solid ${colors.danger}30`, borderRadius: R.sp(10), marginBottom: R.sp(16) }}>
          <div style={{ color: colors.danger, fontSize: R.fs(13) }}>Failed to load train data: {error}</div>
          <button onClick={fetchTrains} style={{ marginTop: R.sp(8), padding: `${R.sp(6)}px ${R.sp(14)}px`, background: colors.danger, color: '#fff', border: 'none', borderRadius: R.sp(6), fontSize: R.fs(12), cursor: 'pointer', fontFamily: 'inherit', minHeight: R.minTouchTarget }}>Retry</button>
        </div>
      )}
      ...
      {view === 'status' && (
        <>
          ...
          {todaySchedule.length > 0 && (
            <div style={{
              padding: R.sp(14), background: `${colors.primary}12`, border: `${R.borderWidth}px solid ${colors.primary}30`,
              borderRadius: R.sp(12), marginBottom: R.sp(16),
            }}>
              <div style={{ color: colors.primaryLight, fontSize: R.fs(11), fontWeight: 600, marginBottom: R.sp(8) }}>YOUR TRAINS TODAY ({todayDay})</div>
              {todaySchedule.map(s => {
                ...
                return (
                  <div key={s.id} style={{
                    ...
                    borderBottom: `${R.borderWidth}px solid ${colors.primary}20`,
                  }}>
                    <div style={{
                      width: R.sp(36), height: R.sp(36), borderRadius: R.sp(8),
                      ...
                      fontSize: R.fs(14), fontWeight: 700, flexShrink: 0,
                    }}>#{s.train}</div>
                    <div style={{ flex: 1 }}>
                      <div style={{ color: colors.text, fontSize: R.fs(13), fontWeight: 500 }}>
                        {s.direction === 'westbound' ? '← West' : 'East →'} · Board at {s.boardStation}
                        {s.exitStation ? ` → ${s.exitStation}` : ''}
                      </div>
                      {result?.station ? (
                        <div style={{ display: 'flex', gap: R.sp(8), marginTop: R.sp(3), flexWrap: 'wrap' }}>
                          <span style={{ fontSize: R.fs(11), color: colors.textSecondary }}>
                          ...
                          {delay !== null && (
                            <span style={{ fontSize: R.fs(11), fontWeight: 600, color: delayColor(delay) }}>
                              {delayText(delay)}
                            </span>
                          )}
                          <span style={{ fontSize: R.fs(11), color: colors.textMuted }}>
                            {result.station.status || ''}
                          </span>
                        </div>
                      ) : (
                        <div style={{ color: colors.textMuted, fontSize: R.fs(11), marginTop: R.sp(3) }}>
            ...
            if (!instances || instances.length === 0) {
              return (
                <div key={num} style={{
                  ...
                  borderRadius: R.sp(12), marginBottom: R.sp(12),
                }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: R.sp(8) }}>
                    <span style={{ color: colors.text, fontSize: R.fs(16), fontWeight: 700 }}>#{num}</span>
                    <span style={{ color: colors.textSecondary, fontSize: R.fs(13) }}>California Zephyr {num === '5' ? '(Westbound)' : '(Eastbound)'}</span>
                  </div>
                  <div style={{ color: colors.textMuted, fontSize: R.fs(12), marginTop: R.sp(8) }}>
              ...
              return (
                <div key={`${num}-${idx}`} style={{
                  ...
                  <button onClick={() => setExpandedTrain(isExpanded ? null : `${num}-${idx}`)} style={{
                    display: 'flex', alignItems: 'center', gap: R.sp(12), width: '100%', padding: R.sp(16),
                    background: 'transparent', border: 'none', cursor: 'pointer', textAlign: 'left',
                    minHeight: R.minTouchTarget,
                  }}>
                    <div style={{
                      width: R.sp(44), height: R.sp(44), borderRadius: R.sp(10),
                      background: num === '5' ? `${colors.secondary}20` : `${colors.accent}20`,
                      display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
                    }}>
                      <span style={{
                        color: num === '5' ? colors.secondary : colors.accent,
                        fontSize: R.fs(18), fontWeight: 700,
                      }}>#{num}</span>
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: R.sp(8) }}>
                        <span style={{ color: colors.text, fontSize: R.fs(15), fontWeight: 600 }}>
                          California Zephyr {num === '5' ? '← West' : 'East →'}
                        </span>
                        {inst.trainState === 'Active' && (
                          <span style={{
                            width: R.sp(8), height: R.sp(8), borderRadius: R.sp(4),
                            background: overallDelay !== null && overallDelay > 30 ? colors.danger : colors.success,
                          }} />
                        )}
                      </div>
                      <div style={{ display: 'flex', gap: R.sp(8), marginTop: R.sp(3), flexWrap: 'wrap' }}>
                        <span style={{ color: colors.textSecondary, fontSize: R.fs(11) }}>
                          {inst.origCode} → {inst.destCode}
                        </span>
                        {currentStation && (
                          <span style={{ color: colors.primaryLight, fontSize: R.fs(11) }}>
                            {currentStation.status === 'Enroute' ? 'En route to' : 'At'} {currentStation.name}
                          </span>
                        )}
                        {overallDelay !== null && (
                          <span style={{ fontSize: R.fs(11), fontWeight: 600, color: delayColor(overallDelay) }}>
                            {delayText(overallDelay)}
                          </span>
                        )}
                      </div>
                      {inst.velocity > 0 && (
                        <div style={{ color: colors.textMuted, fontSize: R.fs(10), marginTop: R.sp(2) }}>
                          {Math.round(inst.velocity)} mph · Heading {inst.heading}
                        </div>
                      )}
                    </div>
                    <span style={{ color: colors.textMuted, fontSize: R.fs(18) }}>{isExpanded ? '▾' : '▸'}</span>
                  </button>

                  {/* Expanded Station List */}
                  {isExpanded && (
                    <div style={{ borderTop: `${R.borderWidth}px solid ${colors.border}`, padding: `${R.sp(8)}px 0` }}>
                      {schedule.filter(s => s.train === num).length > 0 && (
                        <div style={{ padding: `${R.sp(4)}px ${R.sp(16)}px ${R.sp(8)}px`, color: colors.primaryLight, fontSize: R.fs(10), fontWeight: 600 }}>
                      ...
                      {stationList.map((station, si) => {
                        ...
                        return (
                          <div key={si} style={{
                            display: 'flex', alignItems: 'flex-start', gap: R.sp(10),
                            padding: `${R.sp(8)}px ${R.sp(16)}px`,
                            ...
                            {/* Status dot */}
                            <div style={{ marginTop: R.sp(4), flexShrink: 0 }}>
                              <span style={{
                                display: 'block', width: R.sp(10), height: R.sp(10), borderRadius: R.sp(5),
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: R.sp(6) }}>
                                <span style={{
                                  ...
                                  fontSize: R.fs(13), fontWeight: isMyStation ? 600 : 400,
                                }}>{station.name}</span>
                                <span style={{ color: colors.textMuted, fontSize: R.fs(10) }}>({station.code})</span>
                                {isMyStation && (
                                  <span style={{
                                    fontSize: R.fs(9), padding: `1px ${R.sp(5)}px`, borderRadius: R.sp(4),
                                    background: `${colors.primary}30`, color: colors.primaryLight,
                                  }}>YOU</span>
                                )}
                              </div>
                              <div style={{ display: 'flex', gap: R.sp(8), marginTop: R.sp(2), flexWrap: 'wrap' }}>
                                {station.schArr && (
                                ...
                                {station.arr && (
                                  <span style={{ color: colors.textSecondary, fontSize: R.fs(10) }}>
                                ...
                                {delay !== null && (
                                  <span style={{ fontSize: R.fs(10), fontWeight: 600, color: delayColor(delay) }}>
                                    {delayText(delay)}
                                  </span>
                                )}
                              </div>
                              {(station.arrCmnt || station.depCmnt) && (
                                <div style={{ color: colors.textMuted, fontSize: R.fs(10), marginTop: R.sp(1) }}>
                                  {station.arrCmnt || station.depCmnt}
                                </div>
                              )}
                            </div>
                            {/* Status label */}
                            <span style={{
                              fontSize: R.fs(9), padding: `${R.sp(2)}px ${R.sp(6)}px`, borderRadius: R.sp(4), flexShrink: 0,
        ...
        <>
          <div style={{ marginBottom: R.sp(16) }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: R.sp(12) }}>
              <h3 style={{ color: colors.text, fontSize: R.fs(14), fontWeight: 600 }}>MY WORK SCHEDULE</h3>
              <button onClick={() => {
                setEditItem({ id: Date.now(), train: '6', direction: 'eastbound', boardStation: 'LNK', exitStation: '', days: [], note: '' })
                setShowEditSchedule(true)
              }} style={{
                padding: `${R.sp(6)}px ${R.sp(14)}px`, background: colors.gradient1, color: '#fff',
                border: 'none', borderRadius: R.sp(8), fontSize: R.fs(12), fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit',
                minHeight: R.minTouchTarget,
              }}>+ Add</button>
            </div>
                    ...
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: R.sp(8) }}>
                        <span style={{
                          ...
                          fontSize: R.fs(16), fontWeight: 700,
                        }}>#{item.train}</span>
                        <span style={{ color: colors.text, fontSize: R.fs(14), fontWeight: 500 }}>
                          {item.direction === 'westbound' ? '← Westbound' : 'Eastbound →'}
                        </span>
                      </div>
                      <div style={{ color: colors.textSecondary, fontSize: R.fs(12), marginTop: R.sp(4) }}>
                      ...
                      <div style={{ display: 'flex', gap: R.sp(4), marginTop: R.sp(8), flexWrap: 'wrap' }}>
                        {DAYS.map(d => (
                          <span key={d} style={{
                            padding: `${R.sp(3)}px ${R.sp(7)}px`, borderRadius: R.sp(4), fontSize: R.fs(10),
                            ...
                            border: `${R.borderWidth}px solid ${item.days.includes(d) ? colors.primary + '50' : colors.border}`,
                          }}>{d}</span>
                        ))}
                      </div>
                      {item.note && (
                        <div style={{ color: colors.textMuted, fontSize: R.fs(11), marginTop: R.sp(6), fontStyle: 'italic' }}>{item.note}</div>
                      )}
                    </div>
                    <div style={{ display: 'flex', gap: R.sp(4), flexShrink: 0 }}>
          ...
          <div style={{
            ...
            <div style={{ color: colors.warning, fontSize: R.fs(11), fontWeight: 600, marginBottom: R.sp(4) }}>SCHEDULE INFO</div>
      ...
      {showEditSchedule && editItem && (
        <div style={modalOverlay} onClick={() => setShowEditSchedule(false)}>
          <div style={modalContent} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', gap: R.sp(8), marginBottom: R.sp(12) }}>
              {['5', '6'].map(n => (
                <button key={n} onClick={() => setEditItem({
                  ...
                  border: `${R.borderWidth}px solid ${editItem.train === n ? (n === '5' ? colors.secondary : colors.accent) : colors.border}`,
                  borderRadius: R.sp(8), color: editItem.train === n ? '#fff' : colors.textSecondary,
                  fontSize: R.fs(14), fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit',
                  minHeight: R.minTouchTarget,
            ...
            <div style={{ display: 'flex', gap: R.sp(4), marginBottom: R.sp(12), flexWrap: 'wrap' }}>
              {DAYS.map(d => (
                <button key={d} onClick={() => {
                  const days = editItem.days.includes(d) ? editItem.days.filter(x => x !== d) : [...editItem.days, d]
                  setEditItem({ ...editItem, days })
                }} style={{
                  padding: `${R.sp(8)}px ${R.sp(10)}px`, borderRadius: R.sp(6),
                  background: editItem.days.includes(d) ? colors.primary : colors.surfaceLight,
                  border: `${R.borderWidth}px solid ${editItem.days.includes(d) ? colors.primary : colors.border}`,
